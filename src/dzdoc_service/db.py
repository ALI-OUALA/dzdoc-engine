...
def claim_job(
    ...
    for candidate_id, candidate_status, candidate_attempt_count, candidate_started_at in candidates:
        token = secrets.token_hex(24)
        result = session.execute(
            update(Job)
            .where(
                Job.id == candidate_id,
                Job.status == candidate_status,
                Job.attempt_count == candidate_attempt_count,
            )
            .values(
                status="processing",
                attempt_count=candidate_attempt_count + 1,
                lease_token=token,
                lease_expires_at=current + timedelta(seconds=lease_seconds),
                started_at=candidate_started_at or current,
                error_code=None,
                error_message=None,
            )
        )
