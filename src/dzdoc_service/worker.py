    ...
    def run_once(self) -> str | None:
        with self.database.session() as session:
            delivery_info = session.execute(
                select(
                    WebhookDelivery.id, WebhookDelivery.attempt_count, WebhookDelivery.available_at
                )
                .where(
            ...
            if delivery_info is None:
                return None
            delivery_id, attempt_count, old_available_at = delivery_info

            # Optimistic lock: try to claim
            # We push available_at into the future instead of setting status="processing"
            # so that if the worker crashes during the HTTP call, it will eventually retry
            ...
            result = session.execute(
                ...
                .where(
                    ...
                    WebhookDelivery.available_at == old_available_at,
                )
        ...
        # Make blocking HTTP call outside DB session context
