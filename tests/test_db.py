...
from dzdoc_service.db import Base, Job, Tenant, StoredDocument, claim_job, utcnow
import os
...
def test_claim_job_optimistic_concurrency_no_lazy_load():
    ...
    d = StoredDocument(id='d1', tenant_id='t1', sha256='xxx', source_name='x', media_kind='pdf', size_bytes=10, source_object_key='x')
    session.add(t)
    session.add(d)
    ...
    # To test that session.rollback() doesn't evict the remaining candidates (since they are just scalars),
    # we can try to intercept or we can simulate the scenario that fails in ORM objects.
